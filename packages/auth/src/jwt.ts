 ...
 */
function getRequiredSecret(configValue: string | undefined, envKey: string): string {
  const secret = configValue || process.env[envKey];
  if (!secret) {
    throw new Error(
      `[SECURITY] ${envKey} 未配置！JWT 密钥必须通过环境变量或 AuthConfig 提供。` +
      `请在 .env 文件中设置 ${envKey}=<至少32字节的随机字符串>`
    );
  }
  if (secret.length < 32) {
    throw new Error(
      `[SECURITY] ${envKey} 长度不足 32 字节，当前长度: ${secret.length}。` +
      `请使用更安全的密钥。`
    );
  }
  return secret;
