...
- Name: tests/navigation.spec.ts >> Dashboard 页面登录保护 >> 未登录访问 /dashboard/logs 被重定向到登录页
- Location: tests/navigation.spec.ts:53:9

# Error details

```
Error: page.goto: net::ERR_CONNECTION_REFUSED at http://localhost:3000/dashboard/logs
Call log:
  - navigating to "http://localhost:3000/dashboard/logs", waiting until "load"
