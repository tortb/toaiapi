...
export default async function ModelsPage() {
    ...
    return (<SiteShell>
      <section className="max-w-[1100px] mx-auto px-6 py-12">
        ...
        {models.length === 0 ? (<div className="text-gray-500">当前没有可用模型或加载失败，请稍后重试。</div>) : (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {models.map((m) => (<div key={m.id ?? m.name} className="bg-white border border-gray-100 rounded-lg p-4">
