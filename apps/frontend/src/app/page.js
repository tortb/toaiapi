import { IconArrowRight, IconCheck, IconShield, IconCloud, IconBolt, IconStar, IconQuote, IconLogoSmall, LogoOpenAI, LogoAnthropic, LogoGoogle, Isometric3DChart, } from "@/components/PixelIcons";
    ...
    return (<SiteShell>
      ...
      <section className="max-w-[1280px] mx-auto px-6 py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
          <ValueCard icon={<IconShield size={24} className="text-primary"/>} title="统一接入" desc="一次接入,多模型可用。支持 OpenAI、Anthropic、Google 等顶级模型"/>
          <ValueCard icon={<IconBolt size={24} className="text-purple"/>} title="稳定可靠" desc="全球多节点部署,智能路由切换,99.9% 服务可用性保障"/>
          <ValueCard icon={<IconCheck size={24} className="text-success"/>} title="安全保障" desc="企业级安全防护,数据加密传输,安全合规账户管理"/>
          <ValueCard icon={<IconCloud size={24} className="text-info"/>} title="成本优化" desc="灵活的定价策略,相比官方价格最高可节省 70% 成本"/>
        </div>
      </section>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-stretch">
          ...
          <div className="bg-gray-900 rounded-lg overflow-hidden shadow-soft">
            <div className="flex items-center border-b border-gray-800">
              {["cURL", "Python", "JavaScript", "Go", "PHP"].map((t, i) => (<button key={t} className={`px-4 py-2.5 text-[12px] font-medium ${i === 0
  ...
  -d '{
    ...
    "stream": false
          ...
          <div className="flex flex-col justify-center">
            ...
            "多种语言 SDK 支持",
            "实时日志和监控",
        ].map((t) => (<li key={t} className="flex items-center gap-2 text-[13.5px] text-gray-700">
      ...
      <section className="max-w-[1280px] mx-auto px-6 py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
          <PricingCard title="免费试用" price="¥0" unit="新用户专享" features={["免费额度", "体验所有功能", "技术支持"]} buttonText="立即试用" buttonStyle="outline"/>
          <PricingCard title="按量付费" price="¥0.02" unit="/ 1K tokens" features={["无月租费用", "按量计费", "开发者友好"]} buttonText="查看价格" buttonStyle="outline"/>
          <PricingCard title="充值套餐" price="¥100" unit="充值得越多,送得越多" features={["多种套餐可选", "最高赠送 30%", "优先技术支持"]} buttonText="立即充值" buttonStyle="primary" badge="NEW"/>
          <PricingCard title="企业定制" price="定制报价" unit="满足企业个性化需求" features={["专属价格方案", "私有化部署", "专属技术支持"]} buttonText="联系我们" buttonStyle="outline"/>
        </div>
      </section>
      ...
      <section className="max-w-[1280px] mx-auto px-6 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
          <TestimonialCard stars={5} text='"ToAiAPI 的稳定性和速度超出了我们的预期,极大提升了我们的产品体验。"' name="陈伟" role="某 AI Startup 创始人"/>
          <TestimonialCard stars={5} text='"价格合理,服务稳定,技术支持的响应速度也是我们长期合作的原因。"' name="李娜" role="科技公司 CTO"/>
          <TestimonialCard stars={5} text='"接入简单,文档详细,让我们能够快速将 AI 功能集成到产品中。"' name="王芳" role="产品经理"/>
          <TestimonialCard stars={5} text='"多种模型让我们可以灵活选择,统一接口让开发效率大幅提升。"' name="陈晨" role="某科技公司技术负责人"/>
        </div>
      </section>
...
function ValueCard({ icon, title, desc }) {
    return (<div className="bg-white border border-gray-100 rounded-lg p-5 card-hover">
...
function ModelCard({ logo, name, vendor }) {
    return (<div className="bg-white border border-gray-100 rounded-lg p-4 text-center card-hover">
...
function PricingCard({ title, price, unit, features, buttonText, buttonStyle, badge, }) {
    return (<div className="bg-white border border-gray-100 rounded-lg p-6 relative card-hover">
      {badge && (<span className="absolute top-3 right-3 bg-primary text-white text-[10px] font-bold px-2 py-0.5 rounded">
          {badge}
        </span>)}
      ...
      <ul className="space-y-2 mb-5">
        {features.map((f) => (<li key={f} className="flex items-center gap-2 text-[12.5px] text-gray-700">
            <span className="text-primary">
              <IconCheck size={12}/>
            </span>
            {f}
      ...
      <button className={`w-full py-2 rounded text-[13px] font-medium transition ${buttonStyle === "primary"
...
function TestimonialCard({ stars, text, name, role }) {
    return (<div className="bg-white border border-gray-100 rounded-lg p-5">
      <div className="flex items-center gap-0.5 mb-3 text-warning">
        {Array.from({ length: stars }).map((_, i) => (<IconStar key={i} size={12} className="text-warning"/>))}
      </div>
...
function PartnerLogo({ name }) {
    return (<div className="text-[16px] font-bold text-gray-500 tracking-wide">
      {name}
    </div>);
}
...
function SocialIcon({ type }) {
    ...
    return (<a href="#" className="w-7 h-7 rounded-full flex items-center justify-center text-white text-[10px] font-bold" style={{ background: colors[type] }}>
