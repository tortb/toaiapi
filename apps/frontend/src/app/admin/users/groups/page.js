...
import { getUserGroups, createUserGroup, updateUserGroup, toggleUserGroup, deleteUserGroup, } from "@/lib/admin-api";
...
/* ============== 编辑弹窗 ============== */
function GroupFormModal({ group, onSave, onCancel, }) {
        ...
        <form onSubmit={handleSubmit}>
          <div className="px-6 py-4 space-y-4">
            ...
            <div>
              ...
              <input type="text" value={name} onChange={(e) => setName(e.target.value)} disabled={isEdit} required placeholder="vip" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary disabled:bg-gray-50 disabled:text-gray-500"/>
            </div>

            {/* 显示名 */}
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">显示名（中文）</label>
              <input type="text" value={displayName} onChange={(e) => setDisplayName(e.target.value)} required placeholder="VIP 用户" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"/>
            </div>

            {/* 描述 */}
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">描述</label>
              <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={2} placeholder="用户组描述..." className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"/>
            </div>

            {/* 价格倍率 */}
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">价格倍率</label>
              <input type="number" value={priceMultiplier} onChange={(e) => setPriceMultiplier(e.target.value)} min="0.1" max="10" step="0.1" required className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"/>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4">
              <div>
                ...
                <input type="number" value={rpmLimit} onChange={(e) => setRpmLimit(e.target.value)} min="1" required className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"/>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">TPM</label>
                <input type="number" value={tpmLimit} onChange={(e) => setTpmLimit(e.target.value)} min="1" required className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"/>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">最大 Key 数</label>
                <input type="number" value={maxApiKeys} onChange={(e) => setMaxApiKeys(e.target.value)} min="1" required className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"/>
              </div>
            </div>
          </div>

          <div className="px-6 py-4 border-t border-gray-100 flex justify-end gap-3">
            <button type="button" onClick={onCancel} className="px-4 py-2 text-sm text-gray-700 bg-white border border-gray-200 rounded-lg hover:bg-gray-50">
              取消
            </button>
            <button type="submit" className="px-4 py-2 text-sm text-white bg-primary rounded-lg hover:bg-primary-600">
              {isEdit ? "保存" : "创建"}
            </button>
          </div>
        </form>
...
export default function UserGroupsPage() {
    ...
    const [editingGroup, setEditingGroup] = React.useState(undefined);
    ...
    return (<div className="min-h-screen bg-[#F5F7FA] text-gray-900 flex">
      ...
      {/* 编辑弹窗 */}
      {editingGroup !== undefined && (<GroupFormModal group={editingGroup} onSave={handleSave} onCancel={() => setEditingGroup(undefined)}/>)}
            ...
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="flex-1 max-w-md">
                <div className="relative">
                  ...
                  <input type="text" value={searchInput} onChange={(e) => setSearchInput(e.target.value)} placeholder="搜索组名/显示名..." className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"/>
                </div>
              </div>

              {/* 操作按钮 */}
              <div className="flex items-center gap-3">
                <button onClick={fetchGroups} disabled={isLoading} className="px-4 py-2 text-sm border border-gray-200 rounded-lg hover:bg-gray-50 disabled:opacity-50">
                  刷新
                </button>
                <button onClick={() => setEditingGroup(null)} className="px-4 py-2 text-sm text-white bg-primary rounded-lg hover:bg-primary-600">
                  新建分组
                </button>
              </div>
            </div>
            ...
            {isLoading ? (<div className="p-8 text-center">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-[13px]">
                    ...
                    <tbody>
                      {data.items.map((g) => (<tr key={g.id} className="border-b border-gray-50 hover:bg-gray-50">
                          <td className="px-4 py-3">
                            <div>
                              <div className="font-medium text-gray-900">{g.displayName}</div>
                              <div className="text-[11px] text-gray-400 font-mono">{g.name}</div>
                              {g.description && (<div className="text-[11px] text-gray-500 mt-0.5">{g.description}</div>)}
                            </div>
                          </td>
                          <td className="px-4 py-3">
                          ...
                          <td className="px-4 py-3">
                            {g.isBuiltin ? (<span className="inline-flex px-2 py-0.5 rounded text-[11px] font-medium bg-gray-100 text-gray-600">
                                内置
                              </span>) : g.isActive ? (<span className="inline-flex items-center gap-1.5 text-success">
                                <span className="w-1.5 h-1.5 rounded-full bg-success"/>
                                启用
                              </span>) : (<span className="inline-flex items-center gap-1.5 text-gray-400">
                          ...
                          <td className="px-4 py-3 text-right">
                            <div className="flex items-center justify-end gap-2">
                              <button onClick={() => setEditingGroup(g)} className="px-2 py-1 text-[11px] text-primary hover:bg-primary-50 rounded">
                                编辑
                              </button>
                              {!g.isBuiltin && (<>
                                  <button onClick={() => handleToggle(g)} className={`px-2 py-1 text-[11px] rounded ${g.isActive
                        ? "text-warning hover:bg-warning/10"
                        : "text-success hover:bg-success/10"}`}>
                                    {g.isActive ? "禁用" : "启用"}
                                  </button>
                                  <button onClick={() => handleDelete(g)} className="px-2 py-1 text-[11px] text-red-500 hover:bg-red-50 rounded">
                                    删除
                                  </button>
                                </>)}
                            </div>
