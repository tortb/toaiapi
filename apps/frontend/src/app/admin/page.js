...
import { getDashboard, formatAmount, formatNumber, formatDate, getOrderStatusLabel, getChannelStatusLabel, } from "@/lib/admin-api";
import { ToAiAPILogo, IconMenu, IconSearch, IconBell, IconSettings, IconChevronDown, IconRefresh, IconDashboard, IconUserList, IconUserGroup, IconKey, IconOrders, IconRecharge, IconBill, IconInvoice, IconModel, IconChannel, IconPrice, IconSystem, IconLog, IconMonitor, IconBack, IconUsers, IconWallet, IconChartBar, IconChartLine, IconCoin, } from "@/components/PixelIcons";
        ...
        <main className="flex-1 overflow-y-auto p-6">
          ...
          <div className="flex items-end justify-between mb-5">
            ...
            <div className="flex items-center gap-3">
              <button onClick={fetchDashboard} disabled={isLoading} className="bg-primary text-white px-4 py-2 rounded text-[12.5px] font-medium flex items-center gap-1.5 hover:bg-primary-600 disabled:opacity-50">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-5">
            {isLoading ? (Array.from({ length: 5 }).map((_, i) => <SkeletonCard key={i}/>)) : dashboard ? (<>
                <MetricCard icon={<IconUsers size={20} className="text-primary"/>} iconBg="bg-primary-50" label="注册用户" value={formatNumber(dashboard.metrics.totalUsers)} growth={`${dashboard.metrics.totalUsersGrowth > 0 ? "+" : ""}${dashboard.metrics.totalUsersGrowth}%`} positive={dashboard.metrics.totalUsersGrowth >= 0} sub="较上期增长"/>
                <MetricCard icon={<IconWallet size={20} className="text-purple"/>} iconBg="bg-[#F3E5F5]" label="总充值金额" value={`¥ ${formatAmount(dashboard.metrics.totalRecharge)}`} growth={`${dashboard.metrics.totalRechargeGrowth > 0 ? "+" : ""}${dashboard.metrics.totalRechargeGrowth}%`} positive={dashboard.metrics.totalRechargeGrowth >= 0} sub="较上期增长"/>
                <MetricCard icon={<IconChartBar size={20} className="text-info"/>} iconBg="bg-[#E1F5FE]" label="总消费金额" value={`¥ ${formatAmount(dashboard.metrics.totalConsumption)}`} growth={`${dashboard.metrics.totalConsumptionGrowth > 0 ? "+" : ""}${dashboard.metrics.totalConsumptionGrowth}%`} positive={dashboard.metrics.totalConsumptionGrowth >= 0} sub="较上期增长"/>
                <MetricCard icon={<IconChartLine size={20} className="text-orange"/>} iconBg="bg-[#FFF3E0]" label="总调用次数" value={formatNumber(dashboard.metrics.totalRequests)} growth={`${dashboard.metrics.totalRequestsGrowth > 0 ? "+" : ""}${dashboard.metrics.totalRequestsGrowth}%`} positive={dashboard.metrics.totalRequestsGrowth >= 0} sub="较上期增长"/>
                <MetricCard icon={<IconCoin size={20} className="text-success"/>} iconBg="bg-[#E8F5E9]" label="总余额" value={`¥ ${formatAmount(dashboard.metrics.totalBalance)}`} growth="" positive sub="用户总余额"/>
              </>) : null}
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 mb-5">
            ...
            <div className="lg:col-span-8 bg-white rounded-lg p-5 border border-gray-100">
              ...
              {isLoading ? (<div className="h-[240px] bg-gray-50 rounded animate-pulse"/>) : dashboard?.callStats ? (<LineChart data={dashboard.callStats}/>) : (<div className="h-[240px] flex items-center justify-center text-gray-400 text-sm">
                  暂无数据
                </div>)}
            </div>
            ...
            <div className="lg:col-span-4 bg-white rounded-lg p-5 border border-gray-100">
              ...
              {isLoading ? (<div className="h-[240px] bg-gray-50 rounded animate-pulse"/>) : dashboard?.modelDistribution && dashboard.modelDistribution.length > 0 ? (<DonutChart data={dashboard.modelDistribution}/>) : (<div className="h-[240px] flex items-center justify-center text-gray-400 text-sm">
                  暂无数据
                </div>)}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 mb-5">
            ...
            <div className="lg:col-span-8 bg-white rounded-lg p-5 border border-gray-100">
              ...
              {isLoading ? (<div className="space-y-2">
                ...
                </div>) : dashboard?.recentOrders && dashboard.recentOrders.length > 0 ? (<table className="w-full text-[12.5px]">
                  <thead>
                ...
                </table>) : (<div className="py-8 text-center text-gray-400 text-sm">暂无订单</div>)}
            </div>
            ...
            <div className="lg:col-span-4 bg-white rounded-lg p-5 border border-gray-100">
              ...
              {isLoading ? (<div className="space-y-2">
                  {Array.from({ length: 5 }).map((_, i) => (<div key={i} className="h-10 bg-gray-50 rounded animate-pulse"/>))}
                </div>) : dashboard?.channelStatus && dashboard.channelStatus.length > 0 ? (<table className="w-full text-[12.5px]">
                  <thead>
                    <tr className="text-gray-500 text-left">
                ...
                return (<tr key={c.id} className="border-t border-gray-50">
                ...
                </table>) : (<div className="py-8 text-center text-gray-400 text-sm">暂无渠道</div>)}
            </div>
          </div>
        </main>
...
function SkeletonCard() {
    return (<div className="bg-white rounded-lg p-4 border border-gray-100">
...
/* ============== 指标卡片 ============== */
function MetricCard({ icon, iconBg, label, value, growth, positive, sub, }) {
    return (<div className="bg-white rounded-lg p-4 border border-gray-100">
      <div className="flex items-center gap-2 mb-2">
      ...
      {growth && (<div className="flex items-center gap-1.5 text-[11px]">
          <span className={positive ? "text-success" : "text-red-500"}>{growth}</span>
          <span className="text-gray-400">{sub}</span>
        </div>)}
      {!growth && (<div className="text-[11px] text-gray-400">{sub}</div>)}
    </div>);
}
/* ============== 折线图 ============== */
      ...
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-[240px]">
        ...
        {data.map((d, i) => {
            if (i % Math.ceil(data.length / 7) !== 0 && i !== data.length - 1)
                return null;
            return (<text key={i} x={padding.left + i * stepX} y={height - 8} fontSize="10" fill="#8A9BA8" textAnchor="middle">
              {d.label}
            </text>);
        })}
        ...
        {/* 数据点 */}
        {points.map((p, i) => (<rect key={i} x={p.x - 2} y={p.y - 2} width="4" height="4" fill="#fff" stroke="#2962FF" strokeWidth="1.5"/>))}
      </svg>
    </div>);
}
/* ============== 圆环图 ============== */
function DonutChart({ data }) {
    ...
    return (<div>
      <div className="flex items-center justify-center mb-3">
        <div className="relative" style={{ width: size, height: size }}>
          <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ transform: "rotate(-90deg)" }}>
            ...
            const seg = (<circle key={i} cx={center} cy={center} r={radius} fill="none" stroke={CHART_COLORS[i % CHART_COLORS.length]} strokeWidth={strokeWidth} strokeDasharray={`${dash} ${gap}`} strokeDashoffset={-offset}/>);
            offset += dash;
            return seg;
        })}
      ...
      <ul className="space-y-1.5">
        {data.map((d, i) => (<li key={d.name} className="flex items-center gap-2 text-[11.5px]">
