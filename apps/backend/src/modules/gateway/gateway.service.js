...
let GatewayService = (() => {
    ...
    var GatewayService = class {
            ...
            try {
                ...
                for (let i = 0; i < Math.min(channels.length, this.MAX_RETRIES + 1); i++) {
                    ...
                    try {
                        ...
                        lastError = error instanceof Error ? error : new Error(String(error));
                        this.logger.warn(`Stream channel ${channel.channelId} failed: ${lastError.message}`);
                        // 更新渠道统计（失败）
                        await this.channelService.updateChannelStats(channel.channelId, Date.now() - startTime, false).catch(() => { });
                        // 重置计数器准备下一个渠道
                        totalPromptTokens = 0;
         ...
         * 无论流如何终止都必须执行
         */
        async streamCleanup(params) {
            const { apiKey, request, requestPath, startTime, channelId, lastError, totalPromptTokens, totalCompletionTokens, totalTokens, responseContent, } = params;
            const latencyMs = Date.now() - startTime;
            if (lastError) {
                // 所有渠道都失败
                await this.requestLogService.logRequest({
            ...
            // 更新渠道统计（成功）
            await this.channelService.updateChannelStats(channelId, latencyMs, true).catch(() => { });
            ...
            if (finalTotalTokens === 0) {
                ...
                this.logger.warn(`Provider 未返回 usage，使用估算值: prompt=${finalPromptTokens}, completion=${finalCompletionTokens}`);
            }
            // 计算费用并扣减余额
            const cost = await this.billingService.processUsage(apiKey.userId, apiKey.id, channelId, request.model, {
         ...
         * @throws ForbiddenException 模型不在白名单
         */
        checkModelLimit(apiKey, model) {
            if (apiKey.modelLimit.length > 0 &&
                !apiKey.modelLimit.includes(model)) {
                throw new ForbiddenException(`模型 ${model} 不在此 API Key 的允许列表中`);
            }
        }
        /**
         * 估算文本的 Token 数量
