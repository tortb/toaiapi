...
import { PaymentMethodDto } from './dto/create-order.dto';
...
let PaymentService = (() => {
    ...
    var PaymentService = class {
        ...
        constructor(prisma, epayService, alipayService, wechatPayService, paymentConfigService) {
            this.prisma = prisma;
         ...
         * 生成支付链接
         */
        async generatePayUrl(method, order) {
            switch (method) {
         ...
         * @returns 取消的订单数量
         */
        async cancelStaleOrders(timeoutMinutes = 30) {
         ...
         * @param params - 回调参数
         */
        async handlePaymentNotify(method, params, headers) {
            let result;
            // 验证回调
            switch (method) {
                case 'epay':
            ...
            if (result.amount !== undefined && result.amount !== null) {
                ...
                if (Math.abs(notifyAmount - expectedAmount) > 1) {
                    this.logger.error(`Amount mismatch for order ${result.orderNo}: expected ${expectedAmount}, got ${notifyAmount}`);
         ...
         * 判断支付是否成功
         */
        isPaymentSuccess(status, method) {
            if (!status)
                return false;
            switch (method) {
                case 'epay':
